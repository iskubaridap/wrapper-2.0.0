...
var loadPages = (json:Settings) => {
    ...
    let allPagesLoaded = () => {
        ...
        videoElem.addEventListener('canplay', (e) => {
            ...
            ((<any> (document.querySelector('.page[data-id="goToVideoPage"]') as HTMLIFrameElement).contentWindow).document.getElementById('page-video-duration') as HTMLElement).innerText = (duration).toString();
        });
        loader?.classList.add('d-none');
        setResourceBalloon();
        setPage();
    ...
    if(document.getElementById('course-name-txt')) {
        (document.getElementById('course-name-txt') as HTMLElement).innerHTML = `<i class="bi bi-book"></i> &nbsp;${courseName}`;
    }
