...
if((window.location.href).search('/pages') > 0) {
  ...
  if(document.getElementById('video-bookmark') !== null) {
    window.addEventListener('load', (evt)=>{
      loadMe();
    });
    document.getElementById('round-button')?.addEventListener('click', (evt) => {
      togglePlay();
    });
    document.getElementById('round-button-replay')?.addEventListener('click', (evt) => {
      togglePlay();
    });
    document.getElementById('my-playbar')?.addEventListener('click', (evt) => {
      rewindVideo(evt);
    });
    document.getElementById('my-playbar')?.addEventListener('mouseover', (evt) => {
      scrubOn();
    });
    document.getElementById('my-playbar')?.addEventListener('mouseout', (evt) => {
      scrubOff();
    });
    document.getElementById('my-playbar')?.addEventListener('mousemove', (evt) => {
      scrubRoll(evt);
    });
  }
}
